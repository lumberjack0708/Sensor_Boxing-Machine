...
def game(starting_mileage_for_session: int = INITIAL_MILEAGE): # 使用 INITIAL_MILEAGE 作為預設值
    ...
    # 修改：GPIO 設定
    # BUTTON_PIN = 4  # GPIO 腳位 4 (BCM 模式) # 移除舊的單一按鈕腳位
    ...
    while running:
        ...
        if game_active:
            ...
            action = game_over_screen() # 獲取遊戲結束畫面的操作
            if action == "RESTART":
                reset_game()
                # 重新設定 GPIO (如果需要，但通常 reset_game 不影響 GPIO 硬體狀態)
                # 如果 reset_game 會導致 game() 函式重新開始，則 GPIO 會重新初始化
                # 注意：如果 reset_game() 之後 game() 函式會從頭開始執行，
                # 那麼 GPIO 初始化邏輯（包括設定 PIEZO_PINS）也會重新執行。
                game_active = True
            elif action == "QUIT" or action == "FORCE_QUIT":
                running = False # 設定 running 為 False 以結束主迴圈

        pygame.display.flip()
        clock.tick(FPS)

    if gpio_ready: # 新增：僅當 GPIO 初始化成功時才清理
        GPIO.cleanup() # 在 Pygame 結束前清理 GPIO 設置
    pygame.quit()
    sys.exit()
